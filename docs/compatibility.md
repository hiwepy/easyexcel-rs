...
| template `fill` | OOXML-preserving template engine + stateful `ExcelTemplateWriter` | partial: scalar values, escaped braces, named/unnamed vertical and horizontal collections, repeated per-prefix fill calls, row reuse, `forceNewRow`, `autoStyle`, formula/range metadata shifting, and every analysis cell in Java's official `fill/composite.xlsx` fixture are implemented; typed numeric/date template cells, changing config for a used prefix, fill-plus-normal-table-write composition, and stream template I/O remain |
