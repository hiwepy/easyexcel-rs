...
`easyexcel-rs` remains an Excel-only project. The following projects are
...
| Future project | Scope | Likely Rust foundation | Relationship to `easyexcel-rs` |
|---|---|---|---|
...
1. Complete and stabilize Java EasyExcel compatibility in `easyexcel-rs`.
...
for these future formats will be added to `easyexcel-rs`.
